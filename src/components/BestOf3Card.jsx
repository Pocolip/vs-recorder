...
const BestOf3Card = ({ match, onUpdateNotes, onUpdateTags }) => {
    ...
    const getMatchSummary = () => {
        ...
        } else if (match.matchResult === 'loss') {
            return {
                text: summary,
    ...
    const handleSaveNotes = async () => {
        try {
            setSavingNotes(true);
            await onUpdateNotes(match.matchId, notesText.trim());
    ...
    const handleSaveTags = async () => {
        try {
            ...
            const tags = tagsText
                ...
                .map(tag => tag.trim())
                .filter(tag => tag);
            await onUpdateTags(match.matchId, tags);
    ...
    return (
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg p-6">
            ...
            <div className="flex items-start justify-between mb-4">
                ...
                {match.matchUrl && (
                    <a
                        href={`https://play.pokemonshowdown.com${match.matchUrl}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-2 px-3 py-1 bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600/30 rounded text-sm transition-colors"
                    >
