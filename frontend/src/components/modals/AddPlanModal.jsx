// src/components/modals/AddPlanModal.jsx
import React, { useState, useEffect } from 'react';
import { X, AlertCircle, Info } from 'lucide-react';
import PokemonDropdown from '../PokemonDropdown';

/**
 * Modal for adding a new plan (composition) to an opponent team
 ...
 */
const AddPlanModal = ({ onClose, onSubmit, myTeamPokemon = [] }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to add plan. Please try again.');
    } finally {
      setLoading(false);
    }
  };

  // Check if Pokemon is already selected in another position
  const isPokemonDisabled = (pokemon, currentField) => {
    const { lead1, lead2, back1, back2 } = formData;
    const otherSelections = [lead1, lead2, back1, back2].filter(
      (_, idx) => ['lead1', 'lead2', 'back1', 'back2'][idx] !== currentField
    );
    return otherSelections.includes(pokemon);
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-slate-800 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-slate-700">
          <h2 className="text-xl font-semibold text-gray-100">Add Strategy Plan</h2>
          <button
            onClick={onClose}
          ...
          {myTeamPokemon.length === 0 ? (
            ...
            <div className="bg-blue-900/50 border border-blue-500 rounded-lg p-3 flex items-start gap-2">
              ...
              <div className="text-blue-200 text-sm">
                Select 2 Pokemon to lead with and 2 Pokemon to keep in the back. All 4 must be
                unique.
              </div>
            </div>
          )}
